...
public class StudentGUI extends JFrame {
    ...
    private static final String[] NAV_LABELS = {
        "View Tables", "Search Bikes", "Active Rentals",
    ...
    // -- Constructor --------------------------------------------------------------
    public StudentGUI() {
        super("Campus Bike Rental System");
    ...
    private JPanel buildHeader() {
        ...
        JLabel logo = new JLabel("Campus Bike Rentals");
    ...
    private JPanel buildSidebar() {
        ...
        String[] icons = {"List", "Search", "Active", "Rent", "Return"};
    ...
    // =========================================================================
    // CARD 1 – View Table Contents
    // =========================================================================
    private JPanel buildViewPanel() {
        JPanel p = wrapCard("View Table Contents");
    ...
    // =========================================================================
    // CARD 5 – Return a Bike
    // =========================================================================
    private JPanel buildReturnPanel() {
        JPanel p = wrapCard("Return a Bike");
        ...
        JButton returnBtn = styledButton("Return Bike", WARNING);
        returnBtn.setPreferredSize(new Dimension(140, 40));
        gbc.gridx=1; gbc.gridy=2; form.add(returnBtn, gbc);
        ...
        returnBtn.addActionListener(e -> {
            ...
            try {
                ...
                ps2.setString(1, bikeID);
                ResultSet rs2 = ps2.executeQuery(); rs2.next();
                int totalActive = rs2.getInt(1); rs2.close(); ps2.close();
                if (totalActive > 1) { output.setText("Error: Inconsistent data — multiple active rentals for BikeID " + bikeID); setStatus("Inconsistent data.", ERROR); return; }

                // Status is Rented?
                if (!status.equalsIgnoreCase("Rented")) { output.setText("No active rental found."); setStatus("No active rental.", ERROR); return; }

                // Specific rental exists?
                PreparedStatement ps3 = con.prepareStatement("SELECT StartTime FROM Rentals WHERE BikeID = ? AND StudentID = ? AND EndTime IS NULL");
                ps3.setString(1, bikeID); ps3.setString(2, studentID);
                ResultSet rs3 = ps3.executeQuery();
                if (!rs3.next()) { output.setText("No active rental found."); setStatus("No matching rental.", ERROR); rs3.close(); ps3.close(); return; }
                rs3.close(); ps3.close();

                // Update EndTime
                PreparedStatement ps4 = con.prepareStatement("UPDATE Rentals SET EndTime = CURRENT_TIMESTAMP WHERE BikeID = ? AND StudentID = ? AND EndTime IS NULL");
                ps4.setString(1, bikeID); ps4.setString(2, studentID); ps4.executeUpdate(); ps4.close();

                // Update status
                PreparedStatement ps5 = con.prepareStatement("UPDATE Bikes SET Status = 'Available' WHERE BikeID = ?");
                ps5.setString(1, bikeID); ps5.executeUpdate(); ps5.close();
                con.commit();

                output.setText("Success! Bike " + bikeID + " has been returned by student " + studentID + ".");
                setStatus("Return processed successfully.", SUCCESS);
                bikeField.setText(""); studentField.setText("");

            } catch (SQLException ex) {
                output.setText("Error: " + ex.getMessage());
                setStatus("Error returning bike.", ERROR);
                try { con.rollback(); } catch (SQLException ignored) {}
            }
        });
    ...
    private void showLoginDialog() {
        JDialog dialog = new JDialog(this, "Connect to Oracle Database - MUST BE CONNECTED TO MASON VPN", true);
        ...
        gbc.gridwidth=1;
        JTextField userField  = styledField(200);
        JPasswordField passField = new JPasswordField(20);
        stylePasswordField(passField);
        JTextField sqlField   = styledField(200);

        gbc.gridx=0; gbc.gridy=1; panel.add(label("Username:"), gbc);
        gbc.gridx=1;              panel.add(userField,           gbc);
        gbc.gridx=0; gbc.gridy=2; panel.add(label("Password:"), gbc);
        gbc.gridx=1;              panel.add(passField,           gbc);
        gbc.gridx=0; gbc.gridy=3; panel.add(label("SQL Script Path:"), gbc);
        gbc.gridx=1;              panel.add(sqlField,            gbc);

        JLabel hint = new JLabel("e.g. /home/yourusername/bikes_rentals.sql");
        hint.setForeground(TEXT_SEC);
        hint.setFont(new Font("Monospaced", Font.ITALIC, 10));
        gbc.gridx=1; gbc.gridy=4; panel.add(hint, gbc);

        JButton connectBtn = styledButton("Connect", ACCENT);
        ...
        errLabel.setFont(new Font("Monospaced", Font.PLAIN, 12));
        gbc.gridy=6; panel.add(errLabel, gbc);
        ...
        connectBtn.addActionListener(e -> {
            ...
            String sqlPath  = sqlField.getText().trim().replace("\r","");

            if (username.isEmpty() || password.isEmpty() || sqlPath.isEmpty()) {
            ...
            SwingWorker<String, Void> worker = new SwingWorker<>() {
                ...
                protected void done() {
                    try {
                        String result = get();
                        if (result != null) { errLabel.setText(result); connectBtn.setEnabled(true); return; }
                        errLabel.setText("Connected! Loading SQL script...");
                        String scriptResult = executeSQLScript(sqlPath);
                        if (scriptResult != null) { errLabel.setText(scriptResult); connectBtn.setEnabled(true); return; }
                        setStatus("Connected to Oracle DB", SUCCESS);
                        switchCard(0);
                        dialog.dispose();
                    } catch (Exception ex) { errLabel.setText("Error: " + ex.getMessage()); connectBtn.setEnabled(true); }
                }
            };
            worker.execute();
        });
    ...
    // DB METHODS
    // =========================================================================
    private String connectToDatabase(String username, String password) {
    ...
    private String executeSQLScript(String path) {
        File file = new File(path);
        if (!file.exists()) return "SQL file not found: " + path;
        try {
            ...
            for (String raw : sb.toString().split(";")) {
                String sql = raw.trim();
                if (sql.isEmpty() || sql.equalsIgnoreCase("COMMIT")) continue;
                try { stmt.execute(sql); }
                catch (SQLException e) {
                    if (!e.getMessage().toUpperCase().contains("TABLE OR VIEW DOES NOT EXIST"))
                        System.out.println("Script warning: " + e.getMessage());
                }
            }
            con.commit();
            return null;
    ...
    private void populateTable(JTable table, String sql, String[] cols) {
        try (ResultSet rs = stmt.executeQuery(sql)) {
            ...
            while (rs.next()) {
                ...
                for (int i = 0; i < cols.length; i++) {
                    ...
                    row[i] = (val == null) ? "NULL (Active)" : val.toString();
                }
                model.addRow(row);
                count++;
            }
            if (count == 0) showMsg("No records found.", WARNING);
        } catch (SQLException e) { showMsg("Error: " + e.getMessage(), ERROR); }
    }
